# template-users-be - Complete Folder Structure
...
│   │   ├── config.go                              # has KeycloakConfig and Reads from env: KEYCLOAK_BASE_URL, KEYCLOAK_REALM, KEYCLOAK_CLIENT_ID, KEYCLOAK_CLIENT_SECRET
│   │   └── docs/
...
│   │               └── initial_entity_routes_with_auth.go            # Shows public, protected, role-based, permission-based routes
...
│   ├── base/
│   │   ├── deployment.yaml                        # Kubernetes Deployment with placeholders (users, skillsier/users-be:latest, etc.)
...
|-------------|-------------|---------|
| `users` | Service name (kebab-case) | `jobs`, `proposals`, `contracts` |
| `USERS` | Service name (UPPER_CASE) | `JOBS`, `PROPOSALS`, `CONTRACTS` |
| `Users` | Service name (PascalCase) | `Jobs`, `Proposals`, `Contracts` |
| `skillsier.dev/apps/be/users-be` | Go module path | `skillsier.dev/apps/be/jobs-be` |
| `usersdb` | PostgreSQL database name | `jobsdb`, `proposalsdb` |
| `usersuser` | PostgreSQL username | `jobsuser`, `proposalsuser` |
| `postgres://usersuser:PASSWORD@173.212.218.251:5432/usersdb?sslmode=disable` | PostgreSQL connection string | `postgres://user:pass@host:5432/dbname` |
| `173.212.218.251:9092` | Kafka broker addresses | `173.212.218.251:31691` |
| `users` | Kafka topic prefix | `jobs`, `proposals` |
| `8080` | HTTP service port | `8080`, `8081` |
| `skillsier/users-be:latest` | Docker image name | `skillsier/jobs-be:latest` |
| `skillsier` | Kubernetes namespace | `skillsier` |
| `skillsier` | Keycloak realm name | `skillsier` |
| `users-be` | Keycloak client ID | `jobs-be`, `proposals-be` |
