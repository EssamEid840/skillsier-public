...
# Configuration
SERVICE_NAME="<service>-be"
POSTGRES_LOCAL_PORT=5432
KAFKA_LOCAL_PORT=9092
KEYCLOAK_LOCAL_PORT=8080

# K8s configuration
POSTGRES_SERVICE="<service>-be-postgres.skillsier.svc.cluster.local"
POSTGRES_PORT=5432
...
check_secrets() {
  ...
  if [[ -z "${<SERVICE>_BE_DATABASE_PASSWORD}" ]]; then
    missing_secrets+=("DATABASE_PASSWORD")
  fi
  
  if [[ -z "${<SERVICE>_BE_KAFKA_SASL_PASSWORD}" ]]; then
    missing_secrets+=("KAFKA_SASL_PASSWORD")
  fi
  
  if [[ -z "${<SERVICE>_BE_KEYCLOAK_CLIENT_SECRET}" ]]; then
...
setup_port_forwards() {
  ...
  kubectl port-forward -n "$NS_SKILLSIER" "svc/<service>-be-postgres" "${POSTGRES_LOCAL_PORT}:${POSTGRES_PORT}" > /dev/null 2>&1 &
  echo $! >> "$PID_FILE"
...
run_application() {
  ...
  export <SERVICE>_BE_DATABASE_HOST=localhost
  export <SERVICE>_BE_DATABASE_PORT=$POSTGRES_LOCAL_PORT
  export <SERVICE>_BE_KAFKA_BROKERS=localhost:$KAFKA_LOCAL_PORT
  export <SERVICE>_BE_KEYCLOAK_URL=http://localhost:$KEYCLOAK_LOCAL_PORT
  export <SERVICE>_BE_APP_ENVIRONMENT=development
  export <SERVICE>_BE_DATABASE_AUTO_MIGRATE=true
