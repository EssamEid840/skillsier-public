...
COMMENT ON TABLE proposals IS 'Core proposals - maps to internal/domain/proposal/entity.go';
...
CREATE OR REPLACE FUNCTION update_connect_balance()
RETURNS TRIGGER AS $
BEGIN
...
CREATE OR REPLACE FUNCTION on_proposal_submitted()
RETURNS TRIGGER AS $
BEGIN
