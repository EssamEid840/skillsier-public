...
CREATE FUNCTION update_job_search_vector()
RETURNS TRIGGER AS $
BEGIN
    NEW.search_vector :=
...
CREATE FUNCTION update_category_jobs_count()
RETURNS TRIGGER AS $
BEGIN
...
CREATE VIEW v_active_jobs_full AS
SELECT
    j.id AS job_id,
    j.title,
    j.job_type,
...
CREATE VIEW v_jobs_by_category AS
SELECT
    c.id AS category_id,
    c.name AS category_name,
    c.slug AS category_slug,
...
CREATE VIEW v_popular_skills_in_jobs AS
SELECT
    s.id AS skill_id,
